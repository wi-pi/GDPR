...
class PassiveVoice:
    ...
    def index(self, text):
        """Computes the ratio of number of sentences with passive to all sentences

        Accuracy depends on the accuracy of dependency parser in spacy
        ...
        num_all_sents = len(sents)
        print("Number of Sentences = ", num_all_sents)
        if DEBUG:
        ...
        matches = matcher(doc)
        num_passive_sentences = len(matches)
        print("Number of Passive Sentences = ", num_passive_sentences)

        if DEBUG:
            for match in matches:
                print(doc[match[1]:match[2]])
        percentage = 100 * (num_passive_sentences / num_all_sents) if num_all_sents > 0 else 0
        return percentage, num_passive_sentences, num_all_sents


if __name__ == "__main__":
    # Make sure this is run once for all policies to load the model only once.
    p = PassiveVoice()
    print(p.index(Passive))